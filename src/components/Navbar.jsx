...
const Navbar = () => {
  ...
  return (
    <div className="py-2 px-8 flex justify-between items-center font-titleFont z-10 border-b-[1px] border-b-gray-600 bg-bodyColor sticky top-0">
      ...
      <motion.ul
        className="gap-8 sm:flex hidden"
        variants={navVarients}
        initial="hidden"
        animate="visible"
      >
        {navLinksdata.map((link, index) => {
          return (
            <motion.li
              ...
              key={index}
              className={`cursor-pointer hover:text-designColor`}
            >
      ...
      {/* mobile responsive */}
      <motion.ul
        className={`gap-8 sm:hidden flex flex-col bg-gray-900 absolute right-[10%] top-[100%] p-10 rounded-2xl w-[300px]  ${
          toggle ? "flex" : "hidden"
        }`}
      >
        {navLinksdata.map((link, index) => {
      ...
      >
        {toggle ? (
          <h1
            ...
            className="text-[30px] bg-black text-designColor py-2 px-4 rounded-full"
          >
